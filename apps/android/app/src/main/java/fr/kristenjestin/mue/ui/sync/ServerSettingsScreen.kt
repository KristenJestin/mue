 ...
 * ## Two screens in one
 *
 * Paired, it shows the server, the account and `Disconnect server`. Unpaired, it shows the form.
 * There is no third state and no tab: the row in `sync_state` decides, and the same row decides
 * what the section in `Profile` says, so the two can never disagree.
 */
@Composable
...
/** PRD 9.3's half of the screen: what this phone is connected to, and how to stop. */
@Composable
...
@Preview(name = "Server settings — paired", widthDp = 390, heightDp = 1100)
@Composable

 ...
 *
 * `MEAL_SLOTS` is the enum, and it is expected to grow — a fuller set of moments with
 * complete coverage of the day is being written as this lands. So nothing here is typed
 * `Record<MealSlot, …>`: an addition to the enum must not stop the build of a package
 * that has no opinion about how many moments there are.
 *
...
/** PRD_FOOD 10.3's three named windows, transcribed from `MealSlot`'s own constants. */
export const MEAL_SLOT_WINDOWS: readonly MealSlotWindow[] = [
  { slot: "breakfast", fromMinutes: at(5, 0), untilMinutes: at(10, 0) },
  { slot: "lunch", fromMinutes: at(11, 30), untilMinutes: at(14, 30) },
  { slot: "dinner", fromMinutes: at(18, 0), untilMinutes: at(22, 0) },
];

/**
 * The moment every time the named windows leave out belongs to.
 *
 * PRD_FOOD 10.3 makes it the catch-all of the clock, which is what makes
 * [mealSlotForLocalTime] total: there is no time of day it has no answer for, and so no
 * case in which the server would have to refuse a line or invent a moment for it.
 */
export const MEAL_SLOT_FALLBACK: MealSlot = "snack";

/**
 * The middle of each moment, which PRD_FOOD 10.3 makes the default time of a
 * retroactive entry.
 *
...
export function mealSlotForLocalTime(time: string): MealSlot | undefined {
  ...
  const window = MEAL_SLOT_WINDOWS.find(
    (candidate) => minutes >= candidate.fromMinutes && minutes < candidate.untilMinutes,
  );
